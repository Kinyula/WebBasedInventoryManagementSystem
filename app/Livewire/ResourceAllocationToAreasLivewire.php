...
class ResourceAllocationToAreasLivewire extends Component
{
    ...
    use WithPagination;
    public $resource_name, $quantity, $area_of_allocation, $searchAsset = '';


    public function render()
    {
        return view('livewire.resource-allocation-to-areas-livewire', [
            'Assets' => ChasResource::search($this->searchAsset)->where('allocation_status', 'Not Allocated')->get(),
    ...
    public function deleteChasArea($id)
    {
        ...
        $deleteChasArea = AreaOfAllocation::findOrFail($id)->where('college_name', auth()->user()->college_name)->delete();
