...
class ViewAllCiveAssetsLivewire extends Component
{
    ...
    public function render()
    {
        ...
        return view('livewire.view-all-cive-assets-livewire', [
            'Resources' => CiveResource::search($this->civeResourceSearch)->with(
                ['user', 'category']
            )->paginate(15),
    ...
    public function exportCiveResourcesPdf()
    {
        ...
        if (empty($this->resourceId)) {
            ...
            CiveResource::search($this->civeResourceSearch)->with(['user'])->whereIn('id', $this->resourceId)->chunk($chunkSize, function ($data, $dataID) {
    ...
    public function deleteCiveCreatedResources($id)
    {

        $chasResource = CiveResource::findOrFail($id) ? CiveResource::findOrFail($id)->delete() : false;
